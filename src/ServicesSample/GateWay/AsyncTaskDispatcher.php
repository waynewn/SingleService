...
class AsyncTaskDispatcher extends \SingleService\AsyncTaskDispather{
    ...
    public function onServerStart($SingleServer,$swooleRequest)
    {
        ...
        swoole_timer_tick($this->_Config->getIni($this->_Config->getRuntime('CurServModName').'.Timer_Interval_MS'),function ($timer_id, $tickCounter) use ($SingleServer){
                $modName = $this->_Config->getRuntime('CurServModName');
                $max = $this->_Config->getIni($modName.'.Task_Per_Interval');
                \GWLibs\Misc\Funcs::trace("");
                \GWLibs\Misc\Funcs::trace("");
                \GWLibs\Misc\Funcs::trace("on timer start ----------$max");
