...
class SmtpSimple {
    ...
    {
        $_ini=null;
        parse_str($iniString,$_ini);
        if(empty($_ini['port'])){
            $_ini['port']=465;
        }
        $this->setServer($_ini['server'], $_ini['user'], $_ini['pass'], $_ini['port'], $_ini['port']==465); //设置smtp服务器，到服务器的SSL连接
        $this->setFrom($_ini['user']); //设置发件人        
    }
     ...
     */
    public function sendSimple($user,$content,$title)
    {
        $this->_to=array();
        if(is_array($user)){
            foreach ($user as $u){
                $this->setReceiver($u); //设置收件人，多个收件人，调用多次
            }
        }else{
        ...
        $this->setMail($title, $content); //设置邮件主题、内容
        $ret = $this->sendMail(); //发送
        if($ret==false){
    ...
    * @return boolean
    */
    protected function setServer($server, $username="", $password="", $port=25, $isSecurity=false) {
    ...
    * @return boolean
    */
    protected function setReceiver($to) {
        $this->_to[] = $to;
        return true;
    }
    ...
    * @return boolean
    */
    protected function setMail($subject, $body) {
